...
class AuthorBlurbsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        def resp(x):
            return response.xpath(x)

        AuthorBlurbsSpider.biopages = AuthorBlurbsSpider.biopages.union(
            resp(
                # ".quote span a::attr(href)"
                '//*[contains(concat( " ", @class, " " ), concat( " ", "quote"'
                ', " " ))]//span//a/@href'
            ).getall()
        )
        next_page = resp(
            # "li.next a::attr(href)"
            '//*[contains(concat( " ", @class, " " ), concat( " ", "next", " "'
            " ))]//a/@href"
        ).get()
    ...
    def closed(self, reason):
        with open("xpath-scraper-results.json", "r") as f:
            data = load(f)
        try:
            conn = sqlite3.connect("xpath_scrapy_practice.db")
            c = conn.cursor()
            fields = OrderedDict(
                [
                    ("author", "TEXT"),
